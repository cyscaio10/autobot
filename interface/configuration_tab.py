...
class ConfigurationTab:
    ...
    def show_rules(self, group_name):
        ...
        rules_list = tk.Listbox(rules_window, selectmode="multiple")
        rules_list.pack(padx=10, pady=10, fill='both', expand=True)

        # Adicionar regras de exemplo
        rules = ["Rule 1", "Rule 2", "Rule 3", "Rule 4"]
        for rule in rules:
            rules_list.insert(tk.END, rule)

        save_button = tk.Button(rules_window, text="Save", command=lambda: self.save_rules(group_name, rules_list))
        save_button.pack(pady=10)

    def save_rules(self, group_name, rules_list):
        selected_rules